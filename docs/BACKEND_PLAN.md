...
- Backup/restore đã triển khai ở Phase 8 bằng SQLite backup API: tạo file `.sqlite`, validate schema/integrity trước restore, tạo safety backup, restore vào live connection, chạy migrations và reload app state.
...
- Attendance Phase 7A đã persist regular sessions và official `present`/`absent`; roster chính thức trong AttendanceTab được backend lọc theo membership lifecycle cho từng session date.
...
Deliverables Phase 7A (đã hoàn thành):

- Migration `008_attendance`: tạo `attendance_sessions` và `attendance_records`.
- Module `src-tauri/src/attendance/mod.rs`.
- Commands:
  - `get_attendance_week`
  - `set_attendance_status`
  - `toggle_attendance_lock`
...
Phase 9A đã triển khai:

- Nền helper Excel frontend trong `src/lib/excel`.
- Service `saveExcelFile` gọi Rust command `save_excel_file`.
- Export danh sách học sinh từ `StudentListTab`.
