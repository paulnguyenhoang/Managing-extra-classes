...
- Cột hỗ trợ: STT (bị bỏ qua), Họ tên (bắt buộc), Lớp ở trường, Trường, SĐT phụ huynh, Bắt đầu học, Trạng thái (`Đang học`/`Đã nghỉ`/`active`/`paused`, trống = active), Tháng nghỉ (bắt buộc khi Đã nghỉ), Ghi chú. Hai cột tháng `Bắt đầu học` và `Tháng nghỉ` nhận các dạng Excel hay gặp: `MM/YYYY`, `YYYY-MM`, `YYYY-MM-DD`, date cell, Excel serial hoặc `Sep-26`.
...
- Matching KHÔNG dùng STT hay database id: ưu tiên khớp theo họ tên chuẩn hóa + SĐT (chỉ còn chữ số), sau đó họ tên + lớp ở trường + trường. Nếu vẫn không khớp nhưng họ tên là duy nhất trong lớp, import xem đây là học sinh cũ và cập nhật thông tin (giúp sửa trường/lớp/SĐT/ghi chú từ Excel mà không tạo duplicate). Nếu có nhiều học sinh trùng họ tên mà không khớp mạnh được → Bỏ qua (ambiguous); không khớp tên nào → Thêm mới. Dòng update giống hệt DB → Bỏ qua "Không có thay đổi".
- Preview dialog hiển thị file, lớp, số học sinh hiện có trong lớp, số dòng đọc được từ file, số dòng thêm mới/cập nhật/bỏ qua/lỗi/cảnh báo và từng dòng (số dòng Excel, họ tên, hành động, thông báo); các số thêm/cập nhật/bỏ qua chỉ tính trên dòng trong file, không phải tổng học sinh hiện có trong lớp. Xác nhận bằng `"Xác nhận nhập"`, hủy bằng `"Hủy"`.
...
- Nút `"Xuất bảng điểm"` đã có behavior thật từ Phase 9C (`src/features/classes/utils/scoreExport.ts`).
- Xuất đúng bảng điểm đang hiển thị: lớp + tháng đang chọn, roster hợp lệ theo lifecycle, và đúng thứ tự đang nhìn thấy — mặc định sort tên tiếng Việt, hoặc theo cột điểm đang sort (tăng/giảm dần) ở view mode.
- STT trong Excel là `index + 1` sau sort, không dùng database ID; không xuất membershipId/columnId hay bất kỳ id nội bộ nào.
- Cột Excel: STT, Họ tên, rồi một cột cho mỗi bài kiểm tra của tháng theo đúng thứ tự cột hiện tại; điểm là numeric cell, ô trống hiển thị `"-"` giống UI.
- Workbook có metadata (Lớp, Tháng, Ngày xuất, Số học sinh, Số bài kiểm tra, Sắp xếp) và bảng thống kê theo bài kiểm tra: số điểm đã nhập, điểm trung bình (làm tròn 2 chữ số), cao nhất, thấp nhất — bỏ qua ô trống; tháng chưa có bài kiểm tra thì thay bằng ghi chú và vẫn xuất STT + Họ tên.
...
- Validate lớp/tháng TRƯỚC khi ghi: map ẩn so classId/month, không có map thì so metadata `Lớp`/`Tháng`; tháng được normalize từ các dạng Excel hay gặp như `MM/YYYY`, `YYYY-MM`, `YYYY-MM-DD`, date cell/serial hoặc `Sep-26`. Nếu file có map ẩn của tháng cũ nhưng sheet chính đã sửa metadata đúng lớp + đúng tháng đang chọn, import bỏ qua map ẩn cũ và xử lý như file mẫu tháng mới để không dùng nhầm columnId tháng khác. Sai lớp → `"File Excel không đúng lớp hiện tại."`, sai tháng → thông báo rõ tháng trong file và tháng đang chọn, thiếu metadata → `"File Excel thiếu thông tin lớp hoặc tháng."`
...
| Import student Excel | StudentListTab | Native picker + parse ExcelJS + preview dialog, xác nhận gọi `import_students_for_class` (transaction) | `students` + `class_memberships` của lớp đang mở | SQLite | Phase 9D; STT bị bỏ qua, ưu tiên match theo họ tên/SĐT/trường và fallback họ tên duy nhất, không xóa học sinh thiếu trong file |
